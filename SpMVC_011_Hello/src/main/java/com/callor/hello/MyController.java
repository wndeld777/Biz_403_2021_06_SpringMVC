...
public class MyController {
	// 맵핑 주소가 다른 클래스와 같으면 안됌
	// 대신 METHOD가 틀리면 가능
	// 요청 get 인서트 post 업데이트 put 삭제 delete
	/*
	 * RequestMethod
	 * 일반적인 HTTP Server에서
	 * 		GET : 조회(SELECT) - a tag 의 link 클릭
	 ...
	 * 
	 * RequestMapping이 같더라도 RequestMethod가 다르면 같이 사용할수 있다
	 * 
	 * GET method 방식으로 form에 담긴 데이터를 전송할때
	 * http://localhost:8080/hello/insert?name=홍길동&tel=전화번호
	 * - 전송하는 데이터가 주소에 포함되어서 전송되기 때문에
	 * 	데이터가 쉽게 노출될수 있다
	 * - 전체 URI(주소,데이터 등등)의 길이가 256byte 를 넘어가면
	 * 	데이터가 잘리는 현상이 발생한다
	 * 
	 * POST method 방식으로 form 에 담긴 데이터를 전송할때
	 ...
	 * 
	 * 서버입장에서
	 * GET: http://localhost:8080/hello/insert?name=홍길동
	 * 으로 요청을 받았으면
	 * @RequestMapping(value="/insert",method=GET)를 처리하는
	 * method가 있어야 한다.
	 * 
	 * Mapping(value="/insert")가 있는데
	 * method=POST만 있을 경우 서버는 405 Http 상태코드(Status Code)를 응답한다.
	 */
	...
	/*
	 * Sping에서는 Web으로부터 전달받은 데이터를
	 * Java method의 매개변수를 설정하여 받을수 있다.
	 * 
	 * Spring framework 내부에서 Reflection 이라는 코드가 작동되어서
	 * 전달받은 변수이름과 Java method의 매개변수와 이름매칭 방법으로
	 * 변수를 받는다
	 * 이런코드는 개발자가 신경쓰지 않아도 자동으로 작동되는 코드이다
	 ...
	 * 		숫자형, Boolean형 등
	 * 
	 * 설정을 하면 Reflection 이 실행되는 동안에 
	 * 		String 형 데이터를 설정된 데이터 type으로 형변환 과정이 실행된다
	 * 		이 과정에서 형변환의 Exception이 발생하면
	 * 		Reflection 코드가 중단되고
	 * 		Spring Dispatcher는 Web 400 Status Code를 응답해 버린다
	 * 
