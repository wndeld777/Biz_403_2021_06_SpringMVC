 ...
 * 내부에서
 * 요청정보를 Open API로 보내고
 * 수신된 데이터를 DTO(VO)구조에 따라 parsing을 하고
 * 결과를 돌려준다.
 */
...
public class NaverNewsServiceimplV2 extends NaverNewsServiceimplV1 {
	...
	public List<NewsDTO> getNaverList(String queryURL) throws Exception {
		...
		/*
		 * json-simple, Gson 등으로 parsing을 수행할때는 URL 클래스를 사용하여 naver에 요청하고 결과를
		 * jsonString으로 받았다
		 * 
		 * URI 클래스를 사용하여 naver에 요청하는 정보를 생성하기 내부적으로 인코딩등의 문제를 없애기 위해 URL을 사용하지 않고 URI를
		 * 사용한다
		 */
		URI restURI = new URI(queryURL);

		/*
		 * naver에 요청할때 사용할 인증정보를 http protocol의 header에 부착하기
		 */
		...
		// Spring framework 3.2에서 처음 도입된 클래스 4.x에서 본격적으로 사용
		// OpenAPI에 요청을 하기 위한 클래스
		RestTemplate restTemp = new RestTemplate();
		resList = restTemp.exchange(restURI, // naver에 요청할 RUL 정보
