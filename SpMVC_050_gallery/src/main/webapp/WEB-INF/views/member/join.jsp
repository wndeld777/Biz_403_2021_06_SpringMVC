...
<script>
// let user_id = document.querySelector("input#user_id") : id가 있을경우
...
if(user_id){
	...
	// blur(많이사용), focusout event 코드에서
	// alert을 사용하면 lost focus와 alert 사이에서 무한반복이 일어나는
	// 현상이 발생한다.
	// lost focus 가 되었을때 메시지를 사용자에게 보이고 싶을때는
	//		 alert를 사용하지 않고 다른 방법을 강구해야 한다.
	//		 비어있는 div box를 하나 만들고 그곳에
	// 		메시지를 표시하는 방법을 사용한 것
	user_id.addEventListener("blur",(e)=>{
		...
		//})
		.then(response=>response.text()) //위 주석문과 같은 코드
		.then(result=>{
			if(result === "USE_ID"){
