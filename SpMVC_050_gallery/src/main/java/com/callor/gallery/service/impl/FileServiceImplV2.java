...
public class FileServiceImplV2 extends FileServiceImplV1{
	...
	public int delete(String imgFileName) {
		 ...
		 * 파일의 null값 등을 검사하여 실행을 취소하는 방법이 더 편리한 코드가 될 것이다
		 */
		...
		if(delFile.exists()) {
			...
			if(ok) {
				...
				log.debug("파일 삭제 실패");
				/*
				 * method return type 을 wrapper class(Integer,Long) 등으로 설정하면
				 * 실패하는 메시지를 return할때 null 을 return 하면 된다
				 * 
