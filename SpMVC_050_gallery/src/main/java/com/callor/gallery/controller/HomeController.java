...
public class HomeController {
	...
	//Request를 처리할 method
	// a tag를 클릭했을때 : <a href="${rootPath}/dumy/gallery/detail>클릭</a>
	// 주소창에 직접 입력하고 Enter를 눌렀을때 : http://localhost:8080/rootPath/dumy/gallery/detail
	// location.href=${rootPath}/dumy/gallery/detail 로 JS에서 실행했을때
	 ...
	 * </form>
	 * JSP, HTML 에서 위 코드를 만들고 입력화면을 보여준후
	 * 		input box에 어떤 문자열을 입력한 후
	 * 		전송 button을 클릭하면
	 * 이 method가 Request를 수신하고
	 * 		input box에 입력한 문자열은 str 변수에 담기게 된다
	 */
	@RequestMapping(value="/dumy/gallery/detail/image",method=RequestMethod.POST)
	...
	public String home(@RequestParam("one_file") MultipartFile one_file,
		 ...
		 *  또한 input tag에는 multiple="multiple" 이 설정 되어 있어야 한다
		 */
		List<MultipartFile> files = m_file.getFiles("m_file");
