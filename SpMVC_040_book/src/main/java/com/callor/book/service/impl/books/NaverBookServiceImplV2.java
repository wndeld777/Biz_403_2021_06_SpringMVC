 ...
 * getNaverList() method 를 재 정의 하려고 한다
 * 
 * queryURL(), getJsonString() method는 그대로 상속받아 사용하고
 * getNaverList() 만 다시 작성하겠다
 * 
 * ServiceV1의 getNaverLisit()는 json-simple을 사용하여
 * JSON parsing을 수행한 후 List를 return 하는 코드
 * 
 * ServiceV2 에서는 getNaverList() method를 다시 작성하여
...
public class NaverBookServiceImplV2 extends NaverBookServiceImplV1 {
	...
	public List<BookDTO> getNaverList(String jsonString) throws ParseException {
		...
		/*
		 * List는 interface인데 interface는 자신의 type을 가지고 있지 않는 객체인데 Gson을 이용하여 JSON
		 * parsing을 할때 List<DTO> 구조를 알수 있는 방법이 없어서 Gson에서 특별한 객체 생성자를 하나 제공해 주고 이 객체를
		 * 통하여 List<DTO>의 구조를 알 수 있도록 만들어 준다
		 */
