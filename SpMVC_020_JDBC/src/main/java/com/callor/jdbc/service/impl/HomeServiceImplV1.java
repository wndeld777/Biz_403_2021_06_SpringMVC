...
// final 의 생성자를 자동으로 만들어주는 annotation
/*
 * private final, protected final로 선언된 
 * 필드 맴버변수를 초기화하는 코드를 갖는
 * 생성자를 만들어 준다
 */
...
public class HomeServiceImplV1 implements HomeService{
	...
	/*
	 * Main page에서 보여주는 도서정보, 출판사정보, 저자정보 데이터를 각각의 서비스로 부터 수집하기
	 */
	@Override
	public void dashBoard(Model model) {
		...
		/*
		 * model 객체는 Controller에서 사용하는 view를 랜더링하는
		 * 데이터를 담는 도구이다
		 * 
		 * 3개의 Dao로 부터 데이터를 받아서 Controller로 return 하려면
		 * 상당히 불편한 상황이 연출될수 있다
		 * 
		 * 이때,
		 * Controller부터 model 객체를 매개변수로 받아서
		 * model 객체에 Attribute를 추가하면
		 * Controller의 원래 model에도 Attribute가 추가되는 효과를 낼수 있다
		 */
