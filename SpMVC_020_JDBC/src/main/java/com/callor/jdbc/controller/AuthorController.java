...
public class AuthorController {
	...
	public String list(HttpSession hSession, Model model) {
		...
		log.debug("저장정보:{}",auList.toString());
		
		/*
		 * Attribute를 지정할때 흔히 저지르는 실수
		 * Att의 이름은 문자열 형태이기 때문에 어떠한 기호를 사용해도 무방하다
		 * 
		 * 하지만, 
		 * jsp에서 랜더링할때는 이 Attr 문자열이 변수로 취급이 된다
		 * 변수로 취급된다는 것은
		 * java의 일반적인 코딩 규칙이 적용된다는 것이다
		 * 
		 * 작성할때는 비록 문자열이지만, 변수명명규칙에 맞도록 이름을 작성해야 한다
	 ...
	 * default값을 임의로 설정해 두면 코드 내에서 핸들을 할 수 있다
	 */
