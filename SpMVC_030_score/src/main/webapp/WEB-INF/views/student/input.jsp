...
<form id="student_input" method="POST">
	<fieldset>
		<div>
			<label>학번</label>
			 <input name="st_num" id="st_num" value=<c:out value="${STD.st_num }" default="0"/> placeholder="학번을 입력하세요"/>
		</div>
		<div>
			<label>학생이름</label>
...
<script>
	...
	const student_submit = () => {
		...
		if(st_num.value === ""){
			alert("학번은 반드시 입력하세요")
			st_num.focus()
			// event 핸들링 코드에서 코드 진행을 멈출때는 반드시 return false를 한다
			return false;
		}
	...
	document.querySelector("form#student_input").addEventListener("click",(e)=>{
		...
		if(target.tagName === "BUTTON"){
			/*
			tag 에 class를 지정할때 1개만 지정할수도 있지만
			다수의 class를 지정하는 경우도 있다
			이때는 if(target.className === "className") 형식으로 class를 검사할 수 없다
